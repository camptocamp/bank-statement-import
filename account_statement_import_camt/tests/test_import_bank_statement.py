...
def _to_filepath(obj) -> Path:
    ...
    """
    if isinstance(obj, str) and len(Path(obj).parts) == 1:
        obj = Path("account_statement_import_camt") / "test_files" / obj
    return Path(obj)


def _get_files_diffs(parser, inputfile_path, goldenfile_path) -> list:
    """Creates a lists of diffs between ``inputfile_path`` and ``goldenfile_path`` data
...
class TestParser(TransactionCase):
    ...
    def _get_files_diffs(self, inputfile_path, goldenfile_path):
        # Hook method, can be overridden
        return _get_files_diffs(self.parser, inputfile_path, goldenfile_path)

    def _to_filepath(self, file):
        # Hook method, can be overridden
        return _to_filepath(file)
